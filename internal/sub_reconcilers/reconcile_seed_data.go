...
func (r *DatabaseSeedJobReconciler) updateStatusCondition(ctx context.Context, bestie petsv1.Bestie) (ctrl.Result, error) {
	...
	databaseReadyCondition := meta.FindStatusCondition(bestie.Status.Conditions, petsv1.DatabaseReady)
	NewDatabaseCreatedCondition := NewDatabaseCreatedCondition()
	if databaseReadyCondition == nil {
